...
def create_augmented_dataloader(args, dataset):
    ...
    # Select 5,000 random examples from the training set and apply the custom transformation
    ...
    # Tokenize original full training set and the augmented subset
    tokenized_train = dataset.map(tokenize_function, batched=True)
    tokenized_train = tokenized_train.remove_columns(["text"])  # keeps label column
    tokenized_train = tokenized_train.rename_column("label", "labels")
    tokenized_train.set_format("torch")

    augmented_tokenized = augmented_subset.map(tokenize_function, batched=True, load_from_cache_file=False)
    augmented_tokenized = augmented_tokenized.remove_columns(["text"])  # keeps label column
    augmented_tokenized = augmented_tokenized.rename_column("label", "labels")
    augmented_tokenized.set_format("torch")

    # Concatenate original train split with the 5k augmented examples
    full_augmented_train = datasets.concatenate_datasets([tokenized_train["train"], augmented_tokenized])

    # Build dataloader
    train_dataloader = DataLoader(full_augmented_train, shuffle=True, batch_size=args.batch_size)
